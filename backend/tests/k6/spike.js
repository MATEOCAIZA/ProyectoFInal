...
export let options = {
  stages: [
    { duration: "15s", target: 300 }, // Subida brusca a 300 VUs
    { duration: "2m", target: 300 }, // Mantener el pico
    { duration: "15s", target: 0 }, // Descenso a 0
  ],
  thresholds: {
    "http_req_duration{expected_response:true}": ["p(95)<500"], // <500ms en 95%
    http_req_failed: ["rate<0.01"], // menos del 1% fallos
    checks: ["rate>0.99"], // más del 99% checks válidos
  },
  noConnectionReuse: false,
  userAgent: "K6SpikeTester/1.0",
};

const BASE_URL = "http://localhost:3000/api";

// Generar string aleatorio solo con letras
...
const testUsers = Array.from({ length: 100 }, (_, i) => ({
  username: randomLetters(10), // mínimo 9 caracteres
  password: "Test1234!",
...
export function handleSummary(data) {
  ...
  return {
    [`reports/summarySpike-${timestamp}.json`]: JSON.stringify(data, null, 2),
    [`reports/summarySpike-${timestamp}.html`]: generateHTMLReport(data, timestamp),
