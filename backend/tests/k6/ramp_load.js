...
export let options = {
  stages: [
    { duration: "5m", target: 10 }, // inicio suave
    { duration: "5m", target: 50 }, // subir gradualmente
    { duration: "5m", target: 100 }, // llegar al pico
...
export function handleSummary(data) {
  ...
  return {
    [`reports/summaryRamp_load-${timestamp}.json`]: JSON.stringify(data, null, 2),
    [`reports/summaryRamp_load-${timestamp}.html`]: generateHTMLReport(
      data,
      timestamp
    ),
